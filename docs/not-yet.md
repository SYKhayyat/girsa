...
| | Where it is argued |
|---|---|
| **The shell writes Hebrew sentences the reader sees, and works out Hebrew number agreement itself.** Closed, both halves. `93f4979` established that the window says a count as *label: number*; the reopened half — `unfix` returning a Hebrew string and `export_sefer` composing `בלי תיקונים` / `תיקון אחד` / `{n} תיקונים` by hand — is gone from the shell, and the fence was widened from four refusal shapes to `ok_or` / `ok_or_else` / `map_err` producing a literal or a `format!`, which is what let the red build name the fifty-three anonymous sentences and convert each to a `Code`. The one open row under this heading is that the window-facing sentences still go through `refuse(Code::…)` on the Rust side and are translated in `trouble.ts`, so a sentence added outside that seam is caught by the widened guard rather than by review. | [corrections](record/corrections.md#what-has-not-been-checked) |
...
| | Where it is argued |
|---|---|
| **An agent can read your chaburah folders but not shape them.** The 23 August audit's pass over the personal layer gave the MCP end `marks`, `folders`, `queries` and `who_cites` to read, and bookmark and saved-query writes behind the same wall as the notes tools. What it deliberately does not get is folder writing — no creating, no reordering, no taking members out over the wire. A folder's order is the shiur somebody arranged, and an agent reshuffling it silently would be rule 6 with somebody else's hands. Reads report members in order; that is where it stops. | [answering a program](record/answering-a-program.md#writing-and-the-permission-around-it) |
