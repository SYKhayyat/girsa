...
| **NixOS is declared and, until CI ran it, was never entered.** `flake.nix` was written by translating this repository's own Debian package list, which is a good way to be right and no evidence at all. The `nixos` job in `ci.yml` now enters the shell it declares — inside the `nixos/nix` container, so there is no `/usr/lib` to accidentally link against — and runs the window's tests, the bundle and `cargo build --workspace` in it. What that still does not settle is the **window**: a container has no display, so nothing there has opened a WebKitGTK surface, and `WEBKIT_DISABLE_COMPOSITING_MODE` is a line every Tauri application on NixOS carries rather than a line anybody here has watched work. | [`flake.nix`](../flake.nix) |
