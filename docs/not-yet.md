...
| | Where it is argued |
|---|---|
| **The window says `1 arrangements`.** Rust agrees with its numbers: `girsa_plain::said::plural(n, one, many)` and `counted` have been there since the scans work, and its own header records that three composers wrote that ternary eleven times before it was given a name. The window has `fill()`, which substitutes a hole and stops, and **twenty-five** strings in `say.ts` interpolate a count. So both languages are wrong at one, and neither is wrong the same way — English wants a noun with an `s` on it, Hebrew wants a different phrase, and Hebrew's rule is not English's rule spelled differently. Left as a work order rather than fixed at the one place it was noticed, because fixing one of twenty-five is the shape of bug this repository keeps finding a year later. Found by clicking Keep in the arrangements drawer. | [corrections](record/corrections.md#what-has-not-been-checked) |
...
| **The shemos are changed for the six that have a one-letter convention, and left for the two that do not.** יהוה, the אלהים family, אלוה, אל, שדי and צבאות are written with a letter swapped when the setting asks — one letter for one letter, so every mark, link and search hit on the page stays where it was drawn. **אדני and אהיה are untouched**: the conventions for those (`אדנ-י`, spelling it out) all change the length of the line, and there is no swap anybody actually prints. And `אל` and `שדי` are only changed where the text is pointed, because unpointed they are *to* and *my field* and a rule that changed those would rewrite the sefer. | [`girsa_app::shemos`](../crates/girsa-app/src/shemos.rs) |
...
| **The daf turns over at midnight and not at nightfall.** Where nightfall falls is a function of where the reader is standing, and Girsa does not know and will not ask for a location. Otzaria makes the same choice and says nothing about it; here the *next* day's daf is named beside today's, so the few evening hours when the two disagree are visible rather than silently wrong. | [`girsa_app::luach`](../crates/girsa-app/src/luach.rs) |
...
| **The luach knows one limud.** Daf Yomi Bavli, and the shape that carries it takes a list — Mishnah Yomis and Rambam Yomi are schedules of a different kind (a count of mishnayos, a count of perakim) and neither is written. | [`girsa_app::luach`](../crates/girsa-app/src/luach.rs) |
...
| **NixOS is declared and, until CI ran it, was never entered.** `flake.nix` was written by translating this repository's own Debian package list, which is a good way to be right and no evidence at all. The `nixos` job in `ci.yml` now enters the shell it declares — inside the `nixos/nix` image, so there is no `/usr/lib` to accidentally link against — and runs the window's tests, the bundle and `cargo build --workspace` in it. The obvious spelling of that job, `container: nixos/nix`, does not work and the workflow says why: every `uses:` action is a Node program and the runner injects its own glibc Node, which cannot start on Alpine's musl. The **window** was the half this could not reach, on the reasoning that a container has no display — and a container has no display until somebody starts one. `tools/nixos-window.sh` starts `xvfb-run`, opens the binary on it, waits for the screen to stop being blank and counts the colours on it, because the failure it is looking for is not a crash: without `WEBKIT_DISABLE_COMPOSITING_MODE` the surface stays white while the process stays healthy, so a run that lived is evidence of nothing and a picture is. What it does **not** settle is that the window is *right* — nobody is reading a sefer off a colour count. **Nor the AppImage**: `tauri-bundler` downloads `linuxdeploy` at build time — a prebuilt glibc ELF naming `/lib64/ld-linux-x86-64.so.2`, the interpreter this machine does not have — and unlike `node_modules` it arrives during the build and is not there to be patched first, so `--bundles deb` is what the shell's banner says. The .deb and the .rpm are written in Rust and shell out to nothing. | [`flake.nix`](../flake.nix) |
