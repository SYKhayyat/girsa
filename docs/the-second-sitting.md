...
| **Getting it at all** | ~~**F**~~ **A−** — the build that lied refuses, the build that works is proven, and CI attaches installers to a tag. See finding 16 |
...
Neither row is an A, and the reason is the same in both cases: what is left is
not code. *Getting it at all* wants a pushed tag, and *overall* wants an hour of
somebody's actual learning — the thing that produced this document in the first
place, and the one kind of evidence that cannot be generated by the party being
graded.
...
| **Getting it at all** | ~~F~~ **A−** | The build that lied now refuses; the build that works is documented and proven; CI attaches installers to any `v*` tag. **Not an A until a tag is pushed and a reader can click a link** — that is one command, and it is the owner's to run, not an agent's |
...
CI does the same thing on a machine. It has a `bundle` job: the real Tauri build on Windows, the NSIS installer and the MSI
