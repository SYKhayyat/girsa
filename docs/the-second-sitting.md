...
release binary and was identical to the pixel. The second row was a separate
fact and deserved its own line — a reader could not obtain this application, and
...
So the remaining distance is still not code. It is an hour of somebody's actual
learning, on a machine that did not build this — the one kind of evidence that
cannot be generated by the party being graded, and the one that keeps finding
things nothing else does.

| Area | Grade | Why |
|---|---|---|
| Reading a sefer | ~~A−~~ **A** | The daf was already beautiful. What it now also does is give lines back — the window stops at a thousand instead of ending a long sitting holding all 17,418 (finding 22) |
| Speed | A | 11 ms to open an 18,000-segment sefer, 8–90 ms searches, 12 ms shelf tree — and `paint()` no longer walks every line ever drawn |
| The search engine | A− | Fast, honest, badges its guesses, facets with counts, real scope control. **Unchanged, and not mine to raise**: the audit filed no defect against the engine, so this A− is its taste and stands |
| The search *window* | ~~C−~~ **A** | Fixed: it opens in the reader's language, chips and all (findings 7, 20), a mareh makom goes there instead of being searched for (finding 8), and a zero says what it searched rather than showing a blank screen |
| Mefarshim — the flagship | ~~D~~ **A** | Fixed: the comment block is visible for the first time in any build (finding 1), the door counts what it lists (15a), the column lands on the right perek (finding 4), and the marker says how many of *yours* speak — and says nothing at all where that would be true of every line (15d) |
...
| Settings | ~~B~~ **A** | Fixed: both traps. Closing the panel disarms the shortcut it was capturing (finding 13), and the citation style has a control at last — three options, each shown by an example of itself (finding 23) |
...
a guard with them: `collision.test.mjs`, `wire.test.mjs`, the container-query
eye, the four surfaces `sources.test.mjs` now reads, and the runner that makes
the gate one command instead of nine.

Two honest qualifications, because a grade a machine gives its own work is worth
exactly what it is checked against:

* **Nobody has sat with it for an hour.** This document is what an hour produced
  last time, and the value in it was never the list — it was that somebody used
  the application as a person rather than as a test suite. Everything here has
  been measured; none of it has been *lived with*. A third sitting is what turns
  this table from *the defects are closed* into *it is good*.
* **The search engine's A− stands**, because raising a grade nobody filed a
  defect against would be marking one's own homework in the one place there was
  no homework to do.
...
### 1 · Tick a mefaresh, click a line, and nothing appears — **fixed**
...
**Done during the audit itself** — `pane.ts` builds `said-one`, `styles.css`
styles `.said-one + .said-one` — and it is the only change to shipped behaviour
the auditor made, because a guard that lands red is a guard nobody keeps.
`collision.test.mjs` is what stops it recurring: a class whose own bare rule
hides it may be constructed by one module only. See section 25, and finding 15d
for the marker that then marked every line of Bereishis.
...
* ~~**`cargo build --release -p girsa-shell` silently ships a stale frontend.**
  ...
  something else.~~ **Fixed by refusing the build.** The appendix's own
  correction, a day later, found that this command embeds nothing at all — the
  stale frontend was not stale, it was the dev server. So there is nothing to
  relink and nothing to touch: `build.rs` panics on a release profile `tauri`
  has decided is a dev build, and names the two commands that work. Finding 16.
...
### 16 · Nobody had ever built the application — **fixed**
...
**Fixed, and the interesting half is *why it survived*.** Not because the right
command was hard or hidden — `npm run tauri build --no-bundle` worked first try
in 58 seconds. It survived because **the wrong command succeeded**. It printed
`Finished`, wrote an executable, and the executable looked exactly like the
product right up until you unplugged the thing it was quietly leaning on. A
build that fails is a build somebody fixes; a build that lies is a build
everybody trusts, and this one was trusted for the entire life of the project,
including by an audit that opened with a section on how it was tested.

So the wrong command stops succeeding. `app/src-tauri/build.rs` panics on a
release profile that `tauri` has decided is a dev build, and the decision is not
a heuristic: `tauri`'s own build script computes `dev = !custom-protocol` and
publishes it as `DEP_TAURI_DEV`, which is the same one bit the window itself is
compiled against — so the guard cannot disagree with what the binary would have
done. Debug builds are untouched, because `cargo check`, `cargo clippy` and
`tauri dev` all want exactly that binary and CI's shell job runs two of them.
`GIRSA_DEV_RELEASE=1` builds it anyway, for whoever is profiling the release
profile and does not care what the webview points at.

Watched, not reasoned about. `cargo check --release` from `app/src-tauri`:

```
thread 'main' panicked at app\src-tauri\build.rs:43:5:
...
And then the part a guard cannot do, which is produce something. `npx tauri
build`, 19m 03s from cold, produced what this repository had never had:

```
Built application at: target\release\girsa-shell.exe
Finished 2 bundles at:
    target\release\bundle\msi\Girsa_0.1.0_x64_en-US.msi        10.3 MB
    target\release\bundle\nsis\Girsa_0.1.0_x64-setup.exe        7.3 MB
```

`target/release/bundle/` is the directory this finding says does not exist. It
does now, and both installers are in it.

Run with no dev server anywhere, and asked over CDP the way every number in this
document was taken, the binary answers:

```
url  : http://tauri.localhost/
title: גִּרְסָא · Girsa
```

Which is the finding, inverted line for line — it read
`url: "chrome-error://chromewebdata/"`, `title: "localhost"`. Worth recording
that the cheap check was **inconclusive and looked like a failure**: grepping the
executable finds `localhost:5174` present and `pane-body` absent, because Tauri
carries `devUrl` in its embedded config whatever the build is and compresses the
frontend so no class name survives as plain text. A string search on a binary is
not evidence about what that binary does, and reporting it as such would have
been this document's own lesson 3 — a comment defending a behaviour is not
having watched it — with a `grep` in the comment's place.

And then a machine did it, which is the part that makes the finding closed rather
than answered. The `bundle` job built the tag in **15m59s** and put both
installers on
[the v0.1.0 release](https://github.com/SYKhayyat/girsa/releases/tag/v0.1.0):
`Girsa_0.1.0_x64-setup.exe` (7,325,350 bytes) and `Girsa_0.1.0_x64_en-US.msi`
(10,326,016 bytes). `target/release/bundle/` existing on the author's disk is
not the same claim as a reader being able to download something, and this
finding was always about the second one.

The job: the real Tauri build on Windows, the NSIS installer and the MSI
uploaded as artifacts and attached to the release on any `v*` tag, with
`workflow_dispatch` for a build without one. It is deliberately not on every
...
the other two jobs already say. What has to be true on every tag is that a
reader can download something.

The README says all of this in a `Getting it` section, including the part an
installer cannot fix: **Girsa needs a corpus and the installer does not carry
one.** With none, the window now opens on a screen that says so and offers a
folder picker rather than four lines of Latin file paths (finding 19).
...
## Part 7 · What using the shipped build found

Everything above was answered, and then v0.1.0 was **driven** — the release
binary, the real 7,189-work corpus, real mouse and key events over CDP, the same
apparatus the appendix describes. Two thirds of the report re-measured on the
product. Most of it held, and the table in Part 1 says which.

Three things did not, and none of them was findable by reading.

### 26 · A reader who installs Girsa cannot obtain a single sefer

The worst finding in this document, and it was created by fixing finding 16.

The installer works. It carries `girsa-shell.exe` and three licence files —
`bundle.resources` listed exactly those, and `externalBin` was unset. It does
not carry `girsa-fetch`, `girsa-import` or `girsa-index`.

So the road from *I downloaded Girsa* to *I am learning* is:

| | | |
|---|---|---|
| 1 | Fetch Sefaria, 2.2 GB | `girsa-fetch` — **not on the reader's machine** |
| 2 | Obtain Otzaria | **nothing in this repository fetches it**, at all |
| 3 | Build the shelf | `girsa-import` — not on the machine, and it *refuses* without step 2: `read_otzaria` returns `CatalogueError::Missing` unless `<root>/אוצריא` is a directory |
| 4 | Build the index, 3.6 GB | `girsa-index` — not on the machine |

Three of those four binaries exist only in a source checkout. To reach them a
reader installs Rust, clones the repository and builds — at which point they did
not need the installer. And the window's answer to having no corpus was a folder
picker, which asks somebody to choose a directory that cannot exist on their
disk, and says nothing about why.

**This is why the `A` on *Getting it at all* was wrong**, and it is worth being
precise about the error rather than quietly restating the grade. The F said *no
build of this application has ever been produced*. That was fixed, thoroughly:
the build that lied refuses, CI builds the real one, both installers are on the
tag. Then the axis was graded on **the existence of the link** — which is the
question the F asked, and not the question the axis asks. *Getting it at all*
means a reader ends up with Girsa working. The wall behind the wall was never
looked at, by the audit or by me.

Two thirds of it is now fixed, and the third is honest:

* **The tools ship.** `beforeBundleCommand` builds `girsa-fetch`,
  `girsa-import` and `girsa-index` in release, and the bundle installs them to
  `tools/` beside the application. The README's own instructions become
  followable by the person who followed the README.
* **The first screen says the road.** Four numbered steps with the two sizes
  that matter, and — the part a picker can never say — *Otzaria is a download
  this project does not do for you*. A first-run screen that offers only a
  folder picker is a question with no available answer.
* **Step 2 is still manual, and stays that way in this pass.** Automating a
  second corpus's distribution is not a paragraph of code, and pretending a
  button could do it would put the wall one step further back again.

### 27 · A citation opened a second copy of Girsa

Finding 24 listed the `girsa://` scheme as *wired and unproven* — the automated
session's guardrails refused to fire it. Fired here, with Girsa already open:

```
Start-Process "girsa:bavli/shabbat/12b:3#242"
→ two girsa-shell processes
→ the open window still on שינויי נוסחאות על תרומות ג' ג' ה', four tabs untouched
```

So the return half of the Ksav loop — the thing spec.md §10.6 exists for — hands
a reader a **duplicate application** with its own workspace, and the two then
write one `session.json`, last writer winning on everything the reader had open.

On Windows and Linux the deep-link plugin only ever hears a cold start; handing a
URL to a process that is already running is `tauri-plugin-single-instance`'s job,
and it is documented as deep-link's companion on those platforms. It was never
added. With its `deep-link` feature the argv reaches the listener already wired
in `run()`, and the handler raises the window — because a citation that silently
scrolled a window hidden behind your document looks exactly like a citation that
did nothing.

### 28 · Light mode existed, worked, and could not be found

> *"i dont want it stuck in dark mode — there should also be a light mode."*

There was one. Measured on the running build before anything changed: `ערכת
צבעים` offers three, choosing `בהיר` turns the page cream (`#f6f2e8` over
`#1d1a14`), `theme: light` lands in `session.json`, and it survives a restart.
Every part correct.

It was an `<option>` inside a `<select>` inside a panel behind a button, and the
default is *follow the system* — so on a machine whose Windows is dark, Girsa is
dark, and nothing on the reading screen suggests it could be otherwise. **A
feature nobody can find is the same as a feature that is not there**, which is
finding 1's lesson and the old `לצד` button's, arriving a third time in the one
surface a reader looks at for hours.

It is a toolbar round now, beside nikud and showing, keeping their convention —
...
*Ctrl+C does nothing with the search docked* — twice, and both were my own
contamination. The first run clicked the result with a synthesized `.click()`,
which does not move focus the way a mouse does. The second ran against a window
where I had left the settings panel open forty steps earlier, and an open
settings panel holds the keyboard **by design** — `panel.ts` says so in its
header. With real `Input.dispatchMouseEvent` on a freshly restarted window the
answer inverted: focus on `BUTTON.find-hit`, panel `find is-docked`, Ctrl+C →
`הועתק — שינויי נוסחאות על תרומות ג' ג' ה'`.

Which is worth recording next to lesson 1. Giving the gate eyes is not the end of
it: an eye that can drive the product is only as good as its discipline about
state, and a tool that reports what it saw without saying what it had done first
is a tool that manufactures findings. The measurements in this part are the ones
taken from a known state; where I could not establish one, this says so.
