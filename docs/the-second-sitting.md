...
| | |
|---|---|
| **Overall** | ~~**C+**~~ **A−** — every finding closed at the root, and one sitting short of knowing whether it is good |
| **Getting it at all** | ~~**F**~~ **A** — [v0.1.0](https://github.com/SYKhayyat/girsa/releases/tag/v0.1.0), built by a machine, with both installers on it. See finding 16 |
...
*Overall* is not, and the reason is that what is left is not code. It wants an
hour of somebody's actual learning — the thing that produced this document in
the first place, and the one kind of evidence that cannot be generated by the
party being graded.
...
| **Getting it at all** | ~~F~~ **A** | The build that lied now refuses; the build that works is documented; and the link exists — [v0.1.0](https://github.com/SYKhayyat/girsa/releases/tag/v0.1.0) carries `Girsa_0.1.0_x64-setup.exe` (7.3 MB) and `Girsa_0.1.0_x64_en-US.msi` (10.3 MB), built by CI in 15m59s from the tag |
