...
This was `README.md` until **14 August 2026**, kept whole.

It is not a manual and it is not out of date on purpose — it is the **argument**:
...
matched English error prose, the build that lied and succeeded — the reasoning
is here and it is nowhere else.

It moved because a reader who wants to *use* or *build* Girsa should not have to
read 3,300 lines of engineering diary to find `node tools/verify.mjs`. Two
different jobs, two different pages:

| You want | Read |
|---|---|
| to use it, build it, or contribute | [`../README.md`](../README.md), then [`../CONTRIBUTING.md`](../CONTRIBUTING.md) |
| to know **why** something is the way it is | this page |

**Where this page and the README disagree, the README is right.** The counts here
were measured on 14 August 2026 and nothing re-counts them; the README's are
marked and re-counted on every push by
`crates/girsa-app/tests/the_numbers_in_the_readme_are_measurements.rs`. That is
the whole reason the markers were taken off on the way in — a number under a
check that does not read this file is a number wearing the costume of a
measurement.
