...
fn rewrite_metadata(root: &Path, catalogue: &Catalogue) -> (usize, usize) {
    ...
    for work in catalogue.works() {
        ...
        match serde_json::to_vec_pretty(&work) {
            Ok(body) => {
                // Beside-and-rename, like `import::write` does for the same
                // file on a full import: a `--metadata-only` pass is also a
                // writer of `work.json`, and a torn one is the same T1-shaped
                // risk no matter which command tore it.
                if let Err(e) = beside::write(&path, body) {
...
fn write_index(root: &Path, works: &[Work], catalogue: &Catalogue) -> Result<(), std::io::Error> {
    ...
    // Both of these go beside-and-renames. They are the corpus's work index
    // and its Otzaria lexicon — what tells the shelf what is on it — and a
    // truncating write would leave a torn index that reads as a short shelf.
    beside::write(&root.join("works/index.jsonl"), body)?;
...
mod tests {
    ...
    /// The corpus work index and its Otzaria lexicon are written
    /// beside-and-renamed, so a reader never sees a half-written index.
    ///
    /// `write_index` used to truncate with `fs::write`; a torn
    /// `works/index.jsonl` reads back as a short shelf, and a torn lexicon
    /// reads back as a search that cannot find the Otzaria-only works. The
    /// `.part` temp files are the fingerprint of the write being in flight, so
    /// their absence after a completed run — and a second run replacing the
    /// first cleanly rather than appending to it — is the contract the
    /// beside-and-rename doctrine promises.
    #[test]
    fn the_work_index_is_written_beside_and_renamed() {
        let dir = scratch("index-atomic");
        // A catalogue is built from a Sefaria root: one schema and one Hebrew
        // merged text for it to name.
        let sefaria = dir.join("sefaria");
        std::fs::create_dir_all(sefaria.join("schemas")).unwrap();
        // The fetch lays texts out as `json/<category>/Hebrew/merged.json`,
        // and the title key is the directory above `Hebrew`.
        std::fs::create_dir_all(sefaria.join("json/Tanakh/Hebrew")).unwrap();
        std::fs::write(
            sefaria.join("schemas/Tanakh.json"),
            r#"{"title": "Tanakh", "heTitle": "תנ\"ך", "categories": ["Tanakh"]}"#,
        )
        .unwrap();
        std::fs::write(
            sefaria.join("json/Tanakh/Hebrew/merged.json"),
            "{\"text\":[]}",
        )
        .unwrap();
        let (catalogue, skipped) = Catalogue::build(&sefaria, &[]).unwrap();
        assert_eq!(skipped, 0, "the one schema read");
        assert_eq!(catalogue.works().len(), 1);

        let corpus = dir.join("corpus");
        write_index(&corpus, catalogue.works(), &catalogue).unwrap();

        let index = corpus.join("works/index.jsonl");
        let body = std::fs::read_to_string(&index).expect("the index is there");
        assert!(body.contains("Tanakh"), "{body}");
        assert!(
            !corpus.join("works/index.jsonl.part").exists(),
            "no half-written index beside the shelf"
        );
        assert!(
            corpus.join("lexicon-otzaria.tsv").is_file(),
            "the lexicon is written too"
        );
        assert!(
            !corpus.join("lexicon-otzaria.tsv.part").exists(),
            "no half-written lexicon beside the shelf"
        );

        // And a second run replaces the first, so the index never carries a
        // work that was taken off the shelf.
        let first = std::fs::read_to_string(&index).unwrap();
        write_index(&corpus, &[], &catalogue).unwrap();
        let second = std::fs::read_to_string(&index).unwrap();
        assert!(!second.contains("Tanakh"), "{second}");
